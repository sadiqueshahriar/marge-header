...
const models = require('./iswap_api/notusedmodels')
const mongoose = require('mongoose')

const packageConfig = require('./package.json')
const webConfig = require('./assets/json/webConfig.json')
const schemaConfig = require('./iswap_api/config/schema-config.json')
const serverConfig = require('./iswap_api/config/server-config.json')
...
async function configApp() {
    ...
    if (updateJSON('./iswap_api/config/server-config.json', 'server config', update_map.server)) {
...
async function configSchema() {
    ...
    for (const field of Object.keys(answer)) {
        ...
        if (update_map[update_type] == null) {
            update_map[update_type] = {
                [update_key]: answer[field]
            }
        } else {
            update_map[update_type][update_key] = answer[field]
        }
    ...
    updateJSON('./iswap_api/config/schema-config.json', 'schema config', update_map)
    ...
    try {
        ...
        if (REQUEST_CONFIG.token == '') {
            ...
            try {
                ...
                if (req.status == 'ok') {
                    if (await secretKey()) { return } else { return await tryAgain(setSecret) }
                }
            } catch (err) {
        ...
        if (REQUEST_CONFIG.token == '') {
            ...
            try {
                ...
                if (err.error.response != null &&
                    ...
                    if (await adminAccount()) { return } else { return await tryAgain(adminAccount) }
                } else {
...
function isNumber(text) {
    if (text.length == 0 || !isNaN(text)) { return true } else { return 'Please enter a valid number.' }
}

// Check if not empty

function notEmpty(text) {
    if (text.length > 0) { return true } else { return 'Please enter a valid string.' }
}
